    ...
    $.getJSON("/api/formations", (formations) => {
        formations.forEach((formation) => {
            $("#formations").append(`
                <div class="d-flex">
                    <div>
                        ${moment(formation.startedAt).format("YYYY")} - 
                        ${
                            formation.endedAt === null
                                ? "Aujourd'hui"
                                : moment(formation.endedAt).format("YYYY")
                        }
                    </div>
                    <div>
                        <h4>${formation.name}</h4>
                        <span class="text-muted">
                            ${formation.school} - BAC+${formation.gradeLevel}
                        </span>
...
$("body").on("click", ".collection-add", (e) => {
    // console.log(e.currentTarget.dataset.collection);
